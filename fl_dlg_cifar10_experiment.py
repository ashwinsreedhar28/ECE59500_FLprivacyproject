...
def get_cifar_dataloaders(num_clients=5, batch_size=32, iid=True):
...
def get_cifar_test_loader(batch_size=256):
    transform = transforms.Compose([transforms.ToTensor()])
...
def dlg_attack_known_label(
    ...
    for it in range(iters):
        ...
        if (it + 1) % print_every == 0:
            print(f"[DLG-CIFAR10-known-label] Iter {it+1}/{iters}, grad_diff={grad_diff.item():.6f}")
...
def run_experiment_rounds_cifar(
    num_rounds=5,
    num_clients=5,
    ...
    client_loaders = get_cifar_dataloaders(
        num_clients=num_clients,
        batch_size=batch_size,
        iid=True,
    )
    test_loader = get_cifar_test_loader(batch_size=256)
    ...
    for rnd in range(num_rounds):
        print(f"\n=== Federated Round {rnd+1}/{num_rounds} (CIFAR-10) ===")
        ...
        test_acc = evaluate_accuracy(global_model, test_loader, device=device)
        print(f"[Round {rnd+1}] Global test accuracy: {test_acc:.4f}")

        x_recon_all = []
        y_recon_all = []

        for _ in range(num_attacks_per_round):
            ...
            if privacy_cfg is not None and privacy_cfg.use_secure_agg:
                ...
                remaining = [i for i in range(num_clients) if i != victim_idx]
...
def get_experiment_configs(args):
    return {
        ...
        "Adaptive Clip": PrivacyConfig(
            ...
            base_clip_norm=args.base_clip_norm,
            clip_momentum=args.clip_momentum,
        ),
        ...
        "Secure Aggregation": PrivacyConfig(
            use_secure_agg=True,
            secure_agg_group_size=args.secure_agg_group_size,
        ),
...
def plot_all_experiments(results, title, save_plot=None):
    rounds = np.arange(1, len(next(iter(results.values()))["train"]) + 1)

    plt.figure(figsize=(9, 5))
    for name, res in results.items():
        plt.plot(rounds, res["train"], marker="o", linestyle="-", label=f"{name} – Test Acc")
        plt.plot(rounds, res["recon"], marker="x", linestyle="--", label=f"{name} – Recon Acc")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="CIFAR-10 FL + DLG: run all privacy configs and plot together")
    parser.add_argument("--dp_noise_std", type=float, default=0.05, help="Std dev for local DP Gaussian noise")
    parser.add_argument("--base_clip_norm", type=float, default=1.0, help="Base factor for adaptive clipping threshold")
    parser.add_argument("--clip_momentum", type=float, default=0.9, help="EMA momentum for adaptive clipping")
    parser.add_argument("--secure_agg_group_size", type=int, default=3, help="Group size for SecAgg proxy averaging")
    parser.add_argument("--dlg_iters", type=int, default=800, help="DLG optimization iterations")
    parser.add_argument("--num_attacks_per_round", type=int, default=5, help="Number of DLG attacks per round")
    parser.add_argument("--num_rounds", type=int, default=5, help="Number of FL rounds")
    parser.add_argument("--save_plot", type=str, default="", help="Optional path to save plot (e.g., plots/cifar_all.pdf)")
    ...
    for name, cfg in experiment_cfgs.items():
        ...
        train_accs, recon_accs = run_experiment_rounds_cifar(
        ...
        all_results[name] = {"train": train_accs, "recon": recon_accs}

    plot_all_experiments(
        all_results,
        title="CIFAR-10 – Utility vs Leakage Across Privacy Mechanisms",
        save_plot=args.save_plot if args.save_plot else None,
    )
